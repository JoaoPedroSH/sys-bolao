<template>
  ...
  >
    ...
    <div class="vertical-nav-header d-flex items-center ps-6 pe-5 pt-5 pb-2">
      <router-link to="/" class="d-flex align-center text-decoration-none">
        ...
        <v-slide-x-transition>
          <h2 class="app-title text--primary">BOLÃO DOS AMIGOS</h2>
        </v-slide-x-transition>
      </router-link>
    </div>
    ...
    <v-list expand shaped class="vertical-nav-menu-items pr-5">
      ...
      <nav-menu-link
        title="Criar Bolão"
        :to="{ name: 'create_bolao' }"
        :icon="icons.mdiClipboardPlusOutline"
      ></nav-menu-link>

      <nav-menu-group title="Brasileirão" :icon="icons.mdiTournament">
        <nav-menu-link title="Classificação" :to="{ name: 'classification-brazil-a' }"></nav-menu-link>
        <nav-menu-link title="Rodadas" :to="{ name: 'rounds-brazil-a' }"></nav-menu-link>
      </nav-menu-group>
